...
RUN apt-get -y build-dep libopenscenegraph-dev \
    ...
    && mkdir build \
    && cd build \
    && cmake -DBUILD_OSG_PLUGINS_BY_DEFAULT=0 -DBUILD_OSG_PLUGIN_OSG=1 \
...
RUN apt-get -y install libfontconfig1-dev libfreetype6-dev libx11-dev \
...
RUN apt-get -y install libvorbis-dev libmp3lame-dev libopus-dev libtheora-dev \
        libspeex-dev yasm pkg-config libopenjpeg-dev libx264-dev \
    && cd /tmp \
...
RUN apt-get update \
    && apt-get -y install build-essential git wget lsb-release \
        ...
        libncurses5-dev libluajit-5.1-dev libpng12-0 libopus0 \
        libmp3lame0 libtheora0 libfreetype6 \
    && update-alternatives --install /usr/bin/gcc gcc /usr/local/bin/gcc-6 60 \
        --slave /usr/bin/g++ g++ /usr/local/bin/g++-6
